      ...
      },
      {
        ...
        "subParams": [
          ...
          },
          {
            ...
            "displayName": "Include Snowplow Entities in event properties",
          ...
          },
          {
            ...
            "enablingConditions": [],
            "help": "Specifiy the Entity name from the GTM Event, and the key you could like to map it to or leave the mapped key blank to keep the same name. Additionally specify whether to add in event properties or user properties object of the Iterable payload and whether you wish the mapping to apply to all versions of the entity.",
            "valueValidators": []
          },
          {
            "type": "SIMPLE_TABLE",
            ...
            "help": "Specify the Entity name you want to exclude from the Iterable event. Additionally, you can also set whether the exclusion applies to all versions of the entity."
          }
        ]
      }
    ]
...
const mkEventDataFields = (eventData, tagConfig, eventSpFields) => {
  ...
  const final = merge([eventProperties, eventSpFields]);
...
const mkUserDataFields = (eventData, tagConfig, userSpFields) => {
  ...
  const final = merge([userProperties, userSpFields]);
  return cleanObject(final);
};
...
const mkIterableEvent = (eventData, tagConfig, ids, eventSpFields) => {
  const eventBody = {
    ...
    dataFields: mkEventDataFields(eventData, tagConfig, eventSpFields),
  };
...
const mkIterableUserData = (eventData, tagConfig, ids, userSpFields) => {
  const userDataBody = {
    ...
    dataFields: mkUserDataFields(eventData, tagConfig, userSpFields),
  };
...
const trackWithUserIdPath = (iterableEvent, opts, logFun) => {
  logFun('Request', {
    ...
    requestUrl: iterableEndpoints.eventsTrack,
    requestBody: iterableEvent,
  });
  sendHttpRequest(
    ...
    (statusCode, headers, body) => {
      ...
      if (isNoUidResponse(body)) {
        ...
        return trackWithEmailPath(updatedIterableEvent, opts, logFun);
      }
...
const updateWithEmailPath = (
  ...
  logFun('Request', {
    requestMethod: httpOptions.method,
    requestUrl: iterableEndpoints.usersUpdate,
    requestBody: iterableUserData,
  });
  sendHttpRequest(
    ...
    (statusCode, headers, body) => {
      ...
      if (statusCode >= 200 && statusCode < 300) {
        return trackWithEmailPath(iterableEvent, httpOptions, logFun);
      }
      return data.gtmOnFailure();
    },
    httpOptions,
...
const mkLogger = (enabled, stdInfo, invariantInfo) => {
  if (!enabled) {
    return function (logName, logInfo) {
      return; // no-op
    };
  }
  return function (logName, logInfo) {
    const updatedLogInfo = merge([invariantInfo, logInfo]);
    return doLogging(logName, stdInfo, updatedLogInfo);
  };
};
...
const httpOptions = mkRequestOptions(data, false);
const redactedHttpOptions = mkRequestOptions(data, true);
const logging = mkLogger(loggingEnabled, stdLogInfo, {
  requestHeaders: redactedHttpOptions.headers,
});
...
if (!(identifiers.email || identifiers.userId)) {
  if (data.useClientIdFallback) {
    ...
    logging('Message', { msg: 'Unable to derive user identifiers.' });
    return data.gtmOnFailure();
  }
}
...
if (isIdentityEvent(eventData, data)) {
  ...
  if (identifiers.email) {
    return trackWithEmailPath(iterableEvent, httpOptions, logging);
  } else {
    return trackWithUserIdPath(iterableEvent, httpOptions, logging);
  }
}
...
- name: Test defaults
  code: |
    ...
    assertThat(argUrl).isStrictlyEqualTo(trackURL);
    ...
    // assert 'debug' does log in debug
...
- name: Test identifier settings
  code: |
    ...
    const testEvent = mockEventObjectSignUp; // identity event
    const expectedIterableUpdateBody = {
      email: 'foo@bar.baz',
      userId: 'testUser',
      preferUserId: false,
    ...
    const expectedIterableTrackBody = {
      ...
      userId: 'testUser',
      eventName: 'sign_up',
      id: '6926e78c-36ea-4424-b9d0-a014dca402de',
      dataFields: {
        page_location: 'http://localhost:8000/',
        page_encoding: 'UTF-8',
        screen_resolution: '1920x1080',
        viewport_size: '779x975',
        'self_describing_event_com_google_tag-manager_server-side_sign_up_1': {
          method: 'fooMethod',
        },
        'contexts_com_google_tag-manager_server-side_user_data_1': {
        ...
        contexts_com_snowplowanalytics_snowplow_web_page_1: {
          id: '083ecbbc-da82-45e4-b76f-0b3f1902a7da',
        },
      },
    };

    // to assert on
    let updUrl, updCallback, updOptions, updBody;
    let trUrl, trCallback, trOptions, trBody;
    ...
    mock('sendHttpRequest', function () {
      ...
      if (arguments[0] === updateURL) {
        updUrl = arguments[0];
        updCallback = arguments[1];
        updOptions = arguments[2];
        updBody = arguments[3];

        // and call the update callback with mock response
        updCallback(respStatusCode, respHeaders, respBody);
      } else {
        trUrl = arguments[0];
        trCallback = arguments[1];
        trOptions = arguments[2];
        trBody = arguments[3];

        // and call the track callback with mock response
        trCallback(respStatusCode, respHeaders, respBody);
      }
    });
    ...
    // Assert
    // In this test the input is an identity event
    // so we expect sendHttpRequest to have been called twice.
    assertApi('sendHttpRequest').wasCalled();
    ...
    assertThat(updUrl).isStrictlyEqualTo(updateURL);
    assertThat(trUrl).isStrictlyEqualTo(trackURL);

    assertThat(updOptions.method).isStrictlyEqualTo('POST');
    assertThat(updOptions.timeout).isStrictlyEqualTo(5000);
    assertThat(updOptions.headers['Content-Type']).isStrictlyEqualTo(
      'application/json'
    );
    assertThat(trOptions.method).isStrictlyEqualTo('POST');
    assertThat(trOptions.timeout).isStrictlyEqualTo(5000);
    assertThat(trOptions.headers['Content-Type']).isStrictlyEqualTo(
      'application/json'
    );

    const updateBody = jsonApi.parse(updBody);
    assertThat(updateBody).isEqualTo(expectedIterableUpdateBody);

    const trackBody = jsonApi.parse(trBody);
    assertThat(trackBody).isEqualTo(expectedIterableTrackBody);
...
- name: Test identifier settings - useClientIdFallback
  code: |
    ...
    const testEvent = mockEventObjectSignUp; // identity event
    const expectedIterableUpdateBody = {
      email: '443cffcd-5d44-4b64-9606-df47f4fa821e@placeholder.email',
      userId: '443cffcd-5d44-4b64-9606-df47f4fa821e',
      preferUserId: false,
      mergeNestedObjects: false,
      dataFields: {
        phone_number: '+15551234567',
...
- name: Test Snowplow settings - include self-describing
  code: |
    ...
    // assert 'debug' does not log in prod
    assertApi('logToConsole').wasNotCalled();
- name: Test entity rules - include all - edit
  code: |
    const mockData = {
      ...
      useCommonUserId: true,
      useDefaultIdentify: false,
      identityEventsByName: ['media_player_event'], // for testing purposes
    ...
    const expectedIterableUpdateBody = {
      ...
      mergeNestedObjects: false,
      dataFields: {
        user_data_by_rule: { email_address: 'foo@test.io' },
        mobile_context: {
          osType: 'myOsType',
...
- name: Test entity rules - include none - version control
  code: |
    ...
    const expectedIterableUpdateBody = {
      ...
      preferUserId: false,
      mergeNestedObjects: false,
      dataFields: { user_data: { email_address: 'foo@test.io' } },
    };
    const expectedIterableTrackBody = {
      email: 'foo@test.io',
      userId: 'tester',
...
- name: Test entity rules - versioning cases - 2
  code: |
    ...
    const trackBody = jsonApi.parse(trBody);
    assertThat(trackBody).isEqualTo(expectedIterableTrackBody);
- name: Test entity rules - versioning cases - 3
  code: |
    const mockData = {
      apiKey: 'test',
      useClientIdFallback: true,
...
- name: Test additional settings
  code: |
    ...
    // assert 'always' does log in prod
    assertApi('logToConsole').wasCalled();
- name: Test additional settings - identify event
  code: |
    const mockData = {
      apiKey: 'test',
      useClientIdFallback: true,
    ...
    // Identify event
    const testEvent = mockEventObjectIdentify; // identity event
    const expectedIterableUpdateBody = {
      email: 'foo@test.io',
      userId: 'testUser',
      preferUserId: false,
    ...
    const expectedIterableTrackBody = {
      ...
      userId: 'testUser',
      eventName: 'identify',
      id: 'fe0cc7d2-52f3-4ae6-a67b-58c5d4798420',
      dataFields: {
        app: 'testApp',
        self_describing_event_com_snowplowanalytics_snowplow_identify_1: {
          id: 'fooBar',
          email: 'foo@test.io',
        },
        'contexts_com_google_tag-manager_server-side_user_data_1': {
        ...
        contexts_com_snowplowanalytics_snowplow_web_page_1: {
          id: '9ac461c5-10e1-4d19-a4f5-fb29f52ce706',
        },
      },
    };
...
- name: Test logs - Request and Response
  code: |
    // For simplicity reusing the Identify event test
    const mockData = {
      apiKey: 'test',
      useClientIdFallback: true,
      useCommonEmail: true,
    ...
    // Expected to log 4 times
    const expectedUpdateRequestLog = jsonApi.stringify({
      Name: 'Iterable',
      Type: 'Request',
      ...
      RequestBody: expectedIterableUpdateBody,
    });
    const expectedUpdateResponseLog = jsonApi.stringify({
      Name: 'Iterable',
      Type: 'Response',
      TraceId: 'someTestTraceId',
      EventName: 'identify',
    ...
    const expectedTrackRequestLog = jsonApi.stringify({
      Name: 'Iterable',
      Type: 'Request',
      TraceId: 'someTestTraceId',
      EventName: 'identify',
      ...
      RequestBody: expectedIterableTrackBody,
    });
    const expectedTrackResponseLog = jsonApi.stringify({
      Name: 'Iterable',
      Type: 'Response',
      TraceId: 'someTestTraceId',
      EventName: 'identify',
    ...
    assertApi('logToConsole').wasCalled();
    assertApi('logToConsole').wasCalledWith(expectedUpdateRequestLog);
    assertApi('logToConsole').wasCalledWith(expectedUpdateResponseLog);
    assertApi('logToConsole').wasCalledWith(expectedTrackRequestLog);
    assertApi('logToConsole').wasCalledWith(expectedTrackResponseLog);
setup: |-
  const jsonApi = require('JSON');
  const getTypeOf = require('getType');
  const logToConsole = require('logToConsole');

  const updateURL = 'https://api.iterable.com/api/users/update';
  const trackURL = 'https://api.iterable.com/api/events/track';
  ...
  const mockEventObjectSignUp = {
    event_name: 'sign_up',
    client_id: '443cffcd-5d44-4b64-9606-df47f4fa821e',
    language: 'en-US',
    page_encoding: 'UTF-8',
    ...
    'x-sp-app_id': 'testApp',
    'x-sp-platform': 'web',
    'x-sp-dvce_created_tstamp': '1662589318008',
    'x-sp-event_id': '6926e78c-36ea-4424-b9d0-a014dca402de',
    'x-sp-name_tracker': 'testTracker',
    'x-sp-v_tracker': 'js-3.5.0',
    'x-sp-domain_sessionid': 'b54bcc80-96ac-4226-ab2b-602b37b739e2',
    'x-sp-domain_sessionidx': 1,
    'x-sp-br_cookies': '1',
    'x-sp-br_colordepth': '24',
    ...
    'x-sp-doc_width': 779,
    'x-sp-doc_height': 975,
    'x-sp-dvce_sent_tstamp': '1662589318010',
    'x-sp-self_describing_event_com_google_tag-manager_server-side_sign_up_1': {
      method: 'fooMethod',
    },
    'x-sp-contexts_com_google_tag-manager_server-side_user_data_1': [
      {
    ...
    ga_session_id: 'b54bcc80-96ac-4226-ab2b-602b37b739e2',
    ga_session_number: '1',
    'x-ga-mp2-seg': '1',
    'x-ga-protocol_version': '2',
    'x-ga-page_id': '083ecbbc-da82-45e4-b76f-0b3f1902a7da',
  };
  const mockEventObjectIdentify = {
    event_name: 'identify',
    client_id: '443cffcd-5d44-4b64-9606-df47f4fa821e',
    language: 'en-US',
    page_encoding: 'UTF-8',
    ...
    'x-sp-app_id': 'testApp',
    'x-sp-platform': 'web',
    'x-sp-dvce_created_tstamp': '1662594886025',
    'x-sp-event_id': 'fe0cc7d2-52f3-4ae6-a67b-58c5d4798420',
    'x-sp-name_tracker': 'testTracker',
    'x-sp-v_tracker': 'js-3.5.0',
    'x-sp-domain_sessionid': 'c43da118-e237-45e8-8fc3-27b377561882',
    ...
    'x-sp-contexts_com_google_tag-manager_server-side_user_data_1': [
      { email_address: 'foo@test.io', address: { city: 'San Francisco' } },
    ],
    'x-sp-contexts_com_snowplowanalytics_snowplow_web_page_1': [
      { id: '9ac461c5-10e1-4d19-a4f5-fb29f52ce706' },
    ],
    ...
    ga_session_id: 'c43da118-e237-45e8-8fc3-27b377561882',
    ga_session_number: '2',
    'x-ga-mp2-seg': '1',
    'x-ga-protocol_version': '2',
    'x-ga-page_id': '9ac461c5-10e1-4d19-a4f5-fb29f52ce706',
  };
